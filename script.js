...
const systemWallpapers = document.getElementById("systemWallpapers");

function openSystemWallpapers() {
    systemWallpapers.style.display =
        systemWallpapers.style.display === "flex"
        ? "none"
        : "flex";
}
...
function setSystemWallpaper(index) {
    document.querySelector(".background").style.background =
        `url(${wallpapers[index]}) center/cover no-repeat`;

    localStorage.setItem("customBackground", wallpapers[index]);
}
